...
class AccentsAndSyllables:
	...
	def setAccentsAndSyllablesManual(self, wordInfo):
		...
		if wordInfo['sylNum'] == 0 or wordInfo['accentSylNum']:
		...
		while True:			
			...
			(key, extKey) = keyboard.KeyboardWork.getch()
			
			if key == keyboard.KeyboardWork.ENTER_KEY or key == keyboard.KeyboardWork.ENTER_KEY_UNICODE:
				break
						
			if (key == keyboard.KeyboardWork.EXT_KEY_PREFIX and extKey == keyboard.KeyboardWork.ARROW_RIGHT_KEY) or key == keyboard.KeyboardWork.ARROW_RIGHT_KEY_UNICODE:
				curAccentedSylNum = curAccentedSylNum + 1
				if curAccentedSylNum > wordInfo['sylNum']:
					curAccentedSylNum = wordInfo['sylNum']
					
			if (key == keyboard.KeyboardWork.EXT_KEY_PREFIX and extKey == keyboard.KeyboardWork.ARROW_LEFT_KEY) or key == keyboard.KeyboardWork.ARROW_LEFT_KEY_UNICODE:
				curAccentedSylNum = curAccentedSylNum - 1
				if curAccentedSylNum < 1:
					curAccentedSylNum = 1
					
			if keyboard.KeyboardWork.isBreakKeyPressed(key, extKey):            
