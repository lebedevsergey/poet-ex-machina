...
class RhymesAndRitms:
	...
	@staticmethod	
	def isRhyme_OldAlgorithm(wordInfo, rhymedWordInfo):	
		rhymedWord = rhymedWordInfo['word']
		
		# if rhymeType == FEMININE_RHYME:
			# rhymedWord = getConsonantRhyme(rhymedWord['word']) #????
		rhymedWordAccentedSyllOffset = rhymedWordInfo['sylNum'] - rhymedWordInfo['accentSylNum']
		
		rhymedEnd = RhymesAndRitms.getRhymedEnd(rhymedWord, rhymedWordAccentedSyllOffset)
		if not rhymedEnd:
			return False	
		...
		j = len(word) - 1
		for i in range(len(rhymedEnd)-1, -1, -1):
			if j < 0:
				break
			c1 = rhymedEnd[i]
			c2 = word[j]		
...
# 		if len(rhymedEnd1) != len(rhymedEnd2) and not (rhymedWordAccentedSyllOffset == 0 and (len(rhymedEnd1) == 1 or len(rhymedEnd2) == 1)):
	...
	#checks if word can be placed in verse line rhythm	
	@staticmethod
